...
def test_base_import_and_cli_do_not_require_a2a_or_redis():
    script = textwrap.dedent(
        ...
        assert DurableRuntime is not None
        assert ExecutionStore is not None
        assert ExecutionStoreProvider is not None
        assert InMemoryExecutionStoreProvider is not None
        assert RedisExecutionStoreProvider is not None
