...
| [async_hybrid_streaming](./pipeline_wrappers/async_hybrid_streaming/) | Pipeline with legacy sync-only components using hybrid mode | • Pipeline with a sync-only component (one without `run_async`)<br/>• `allow_sync_streaming_callbacks=True`<br/>• Automatic sync-to-async bridging<br/>• Migration example | Using sync-only components in async pipelines, migrating from sync to async gradually, handling third-party sync-only components |
...
| [a2a_long_running](./a2a_long_running/) | Recoverable OpenAI agent over A2A | • `OpenAIChatGenerator`-based Haystack Agent<br/>• In-process execution with Redis Stream checkpoints<br/>• Hayhooks and client restart recovery<br/>• `input-required` continuation<br/>• Polling and durable cancellation | Building tool-using A2A agents whose accepted work resumes after process restarts |
...
| [durable_chat_with_website](./durable_chat_with_website/) | Restart-safe website Q&A | • Familiar fetcher/converter/prompt/LLM graph<br/>• Typed durable REST endpoint<br/>• Pipeline snapshot recovery<br/>• Bounded retry and cancellation | Applying durable execution to the standard `chat_with_website` example |
