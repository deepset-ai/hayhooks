...
- **Use async methods.** Implement `run_api_async()` (or `run_chat_completion_async()`) and run your `Pipeline` with `run_async()`. This lets a single worker handle many concurrent requests because it can switch to another request while waiting on I/O.
