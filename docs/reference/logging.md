...
class PipelineWrapper(BasePipelineWrapper):
    ...
    def run_api(self, query: str) -> str:
        log.debug("Processing query: {query}", query=query)
        try:
            result = self.pipeline.run({"prompt": {"query": query}})
            log.info("Pipeline execution completed successfully")
            return result["llm"]["replies"][0]
        except Exception as e:
            log.error("Pipeline execution failed: {e}", e=e)
            raise
```
...
class PipelineWrapper(BasePipelineWrapper):
    def run_api(self, query: str) -> str:
        ...
        log.info("Pipeline executed in {execution_time:.2f} seconds", execution_time=execution_time)
