...
A deployed pipeline is exposed as an A2A agent when it implements `run_chat_completion` or `run_chat_completion_async` — the same methods used by the [OpenAI-compatible chat endpoints](openai-compatibility.md). No extra method is needed.
...
4. The task ends in `completed` (or `failed`, with the error in the status message — enable `HAYHOOKS_SHOW_TRACEBACKS` to include tracebacks).
