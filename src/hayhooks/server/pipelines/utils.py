    ...
    for name, component in pipeline.walk():
        ...
        if hasattr(component, "run"):
            ...
            if "streaming_callback" in sig.parameters:
                log.trace(
                    "Streaming component found in '{name}' with type {type(component)}", name=name, component=component
                )
...
def _setup_streaming_callback_for_pipeline(
    ...
    if streaming_components == "all":
        ...
        log.trace("Streaming enabled for all components via 'all' keyword", components=components_to_stream)
    ...
    # Default behavior: stream only the last capable component
        ...
        if all_streaming_components:
            ...
            log.trace("Streaming enabled for last component only", component=all_streaming_components[-1][1])
    ...
    # Use explicit list of component names
        ...
        log.trace("Streaming enabled for components", components=[name for _, name in components_to_stream])
    ...
    for _, component_name in components_to_stream:
        ...
        log.trace("Streaming callback set for component", component_name=component_name)
...
def _process_pipeline_end(result: dict[str, Any], on_pipeline_end: OnPipelineEnd) -> Optional[StreamingChunk]:
    ...
    if on_pipeline_end:
        try:
            ...
            log.error("Error in on_pipeline_end callback", exc_info=True, error=e)
    return None
...
def streaming_generator(  # noqa: PLR0913
    ...
    *,
    ...
    try:
        ...
        log.trace("Streaming pipeline run args", args=configured_args)
    except Exception as e:
        log.error("Error in streaming callback setup", exc_info=True, error=e)
        raise
    ...
    def run_pipeline() -> None:
        try:
            ...
            log.error("Error in pipeline execution thread for streaming_generator", exc_info=True, error=e)
...
def _create_hybrid_streaming_callback(
    ...
    if has_async_support:
        ...
        log.trace("Using async streaming callback for component", component_name=component_name)
        ...
        log.trace("Using sync streaming callback (hybrid mode) for component", component_name=component_name)
...
def _setup_hybrid_streaming_callbacks_for_pipeline(
    ...
    if streaming_components == "all":
        components_to_stream = all_streaming_components
        log.trace("Hybrid streaming enabled for all components via 'all' keyword", components=components_to_stream)
    elif streaming_components is None:
        if all_streaming_components:
            components_to_stream = [all_streaming_components[-1]]
            log.trace("Hybrid streaming enabled for last component only", component=all_streaming_components[-1][1])
    elif isinstance(streaming_components, list):
        enabled_component_names = set(streaming_components)
        for component, component_name in all_streaming_components:
            if component_name in enabled_component_names:
                components_to_stream.append((component, component_name))
        log.trace("Hybrid streaming enabled for components", components=[name for _, name in components_to_stream])
    ...
    for component, component_name in components_to_stream:
        ...
        pipeline_run_args[component_name]["streaming_callback"] = streaming_callback
        log.trace("Hybrid streaming callback set for component", component_name=component_name)
...
async def _stream_chunks_from_queue(
    ...
    while not pipeline_task.done() or not queue.empty():
        ...
        try:
            ...
            log.error("Unexpected error in async streaming generator", error=e)
            raise e
...
async def _cleanup_pipeline_task(pipeline_task: asyncio.Task) -> None:
    ...
    if not pipeline_task.done():
        ...
        try:
            ...
            log.warning("Error during pipeline task cleanup", error=e)
            raise e
