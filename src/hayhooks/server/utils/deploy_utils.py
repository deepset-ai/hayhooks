...
from hayhooks.server.durable.routes import remove_durable_api_routes as _remove_durable_api_routes
...
) -> DurableDeployment | None:
    if runtime is not None:
        return runtime.create_deployment(name, wrapper)
    if durable_authoring_mode(wrapper) is not DurableAuthoringMode.NONE:
        msg = "Durable pipeline deployment requires an application-owned DurableRuntime"
...
class _DeploymentSnapshot:
    ...
    def refresh_publication(self) -> None:
        """Capture unrelated route changes made while this pipeline prepared."""
        if self.app is not None:
            self.routes = list(self.app.routes)
...
async def _deploy_prepared_pipeline_async(  # noqa: C901, PLR0912, PLR0913, PLR0915
    pipeline_name: str,
    prepare: Callable[[], Awaitable[PreparedPipeline]],
    *,
    app: FastAPI | None,
    durable_runtime: DurableRuntime | None,
    overwrite: bool,
    remove_files_before_prepare: bool,
    ...
    dlog = log.bind(
        ...
        overwrite=overwrite,
        deploy_concurrency=settings.deploy_concurrency.value,
    )
        ...
        try:
            async with _deployment_lock(_deployment_publication_lock):
                ...
                if snapshot.deployment is not None:
                    await snapshot.deployment.quiesce()
                    old_quiesced = True
                    await snapshot.deployment.close()
                    nonterminal = await _durable_nonterminal_count(snapshot.deployment)
                    if nonterminal:
                        # ponytail: reject all live-work replacements; isolated candidate imports can restore
                        # same-revision hot swaps if that capability becomes necessary.
                        dlog.bind(nonterminal=nonterminal).debug(
                            "Rejected pipeline replacement while durable work remains"
                        )
                        msg = (
                            f"Pipeline '{pipeline_name}' has {nonterminal} nonterminal durable execution(s); "
            ...
            if candidate is not None and durable_runtime is not None and durable_runtime.started:
            ...
            async with _deployment_lock(_deployment_publication_lock):
                ...
                if candidate is not None and durable_runtime is not None and durable_runtime.started:
                    candidate.activate()
                dlog.debug("Published pipeline deployment")
                return result
                ...
                async with _deployment_lock(_deployment_publication_lock):
                    try:
                        ...
                        if (
                            ...
                            and durable_runtime is not None
                            and durable_runtime.started
                        ):
...
async def deploy_pipeline_yaml_async(  # noqa: PLR0913 - stable public deployment API
    pipeline_name: str,
    source_code: str,
    ...
    *,
    ...
    return await _deploy_prepared_pipeline_async(
        pipeline_name,
        lambda: _offload(
        ...
        durable_runtime=durable_runtime,
        overwrite=overwrite,
        remove_files_before_prepare=overwrite and save_file,
        cleanup_files_on_overwrite=overwrite and not save_file,
    )


async def deploy_pipeline_files_async(  # noqa: PLR0913 - stable public deployment API
    pipeline_name: str,
    files: dict[str, str],
    app: FastAPI | None = None,
    ...
    *,
    ...
    return await _deploy_prepared_pipeline_async(
        ...
        durable_runtime=durable_runtime,
        overwrite=overwrite,
        remove_files_before_prepare=overwrite and save_files,
        cleanup_files_on_overwrite=overwrite and not save_files,
    )


async def undeploy_pipeline_async(
    pipeline_name: str,
    app: FastAPI | None = None,
    *,
    durable_runtime: DurableRuntime | None = None,
    ...
    async with policy_lock, _deployment_lock(_deployment_publication_lock):
        ...
        if deployment is not None:
            ...
            try:
                ...
                if await _durable_nonterminal_count(deployment):
                    raise HTTPException(
                        status_code=409,
                        detail=(
                            f"Pipeline '{pipeline_name}' has durable executions that must be completed or canceled "
                            "before undeployment"
                        ),
                    )
            except BaseException:
                if durable_runtime is not None and durable_runtime.started:
...
async def _durable_nonterminal_count(deployment: DurableDeployment) -> int:
    """Return the authoritative number of executions that would be stranded."""
    counts = await deployment.store.operational_counts()
    return int(counts.get("nonterminal", 0))
...
def add_pipeline_api_route(
    ...
    *,
    ...
    if deployment is None:
        ...
        deployment = _deployment_candidate(runtime, pipeline_name, pipeline_wrapper)
        if runtime is not None:
            runtime.install_deployment(pipeline_name, deployment)
...
def deploy_pipeline_files(  # noqa: PLR0913 - stable public deployment API
    pipeline_name: str,
    files: dict[str, str],
    app: FastAPI | None = None,
    save_files: bool = True,
...
def deploy_pipeline_yaml(  # noqa: PLR0913 - stable public deployment API
    pipeline_name: str,
    source_code: str,
    app: FastAPI | None = None,
    overwrite: bool = False,
...
def deploy_pipelines(*, durable_runtime: DurableRuntime | None = None) -> None:
...
def undeploy_pipeline(pipeline_name: str, app: FastAPI | None = None) -> None:
    ...
    with trace_operation(
        ...
        if app:
            route_paths = {f"/{pipeline_name}/run"}
            app.routes[:] = [
