...
from hayhooks.server.a2a.runtime import A2ARuntime, TaskAwareRequestContextBuilder, create_task_store_provider
...
def _create_agent_mount(pipeline_name: str, base_url: str, runtime: A2ARuntime) -> Mount:
    ...
    request_handler = DefaultRequestHandler(
        ...
        agent_card=card,
        request_context_builder=TaskAwareRequestContextBuilder(task_store),
    )
    routes = [
...
def _create_app_task_store_provider(durable_agents_deployed: bool) -> TaskStoreProvider:
    ...
    if backend == "auto" and durable_redis:
        backend = "redis"
...
def create_a2a_app(*, base_url: str | None = None, debug: bool = False, runtime: A2ARuntime | None = None) -> Starlette:
    ...
    durable_agents_deployed = any(
        durable_authoring_mode(wrapper) is DurableAuthoringMode.MANAGED_AGENT
    ...
    if not agent_names:
        log.warning(
            ...
            "A pipeline must expose a managed durable Agent or implement run_chat_completion "
