...
from hayhooks.server.a2a.imports import AgentCapabilities, AgentCard, AgentInterface, AgentSkill
...
def is_a2a_exposable(pipeline_name: str) -> bool:
    ...
    A pipeline is exposable when it provides a durable Agent or implements
    ``run_chat_completion`` / ``run_chat_completion_async`` and does not set
    ``skip_a2a = True``.
    """
    ...
    exposable = (
        durable_authoring_mode(pipeline_wrapper) is DurableAuthoringMode.MANAGED_AGENT
