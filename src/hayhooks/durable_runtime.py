...
from hayhooks.durable_adapters import HaystackDurableAdapter, definition_revision, execution_kind
...
from hayhooks.redis_execution import RedisExecutionStoreProvider
...
class DurableDeployment:
    ...
    def __init__(self, name: str, wrapper: BasePipelineWrapper, provider: DurableExecutionStoreProvider) -> None:
        ...
        self.builtin_agent = (
            kind is ExecutionKind.AGENT
            and bool(getattr(wrapper, "durable", False))
            and not getattr(wrapper, "_is_run_durable_implemented", False)
            and not getattr(wrapper, "_is_run_durable_async_implemented", False)
        )
    ...
    async def get(
        ...
        *,
        ...
        if record.deployment_name != self.name:
            raise KeyError(execution_id)
...
class DurableRuntime:
    ...
    def has_capability(self, wrapper: BasePipelineWrapper) -> bool:
        return bool(
            ...
            or (getattr(wrapper, "durable", False) and wrapper.pipeline is not None)
        )
