...
class RedisExecutionStore:
    ...
    async def submit(self, record: ExecutionRecord) -> bool:
        ...
        return bool(
            await self._scripts.submit(
                keys=[self._record_key(record.execution_id), self.stream_key, self.state_counts_key],
                args=[record.to_json(), record.execution_id],
            )
        )
    ...
    async def resume(self, execution_id: str, update: JsonValue | None = None) -> bool:
        ...
        return bool(
            await self._scripts.resume(
                keys=[self._record_key(execution_id), self.stream_key, self.state_counts_key],
    ...
    async def retire_incompatible(self, definition_revision: str) -> int:
        ...
        retired = 0
        cursor: int | bytes = 0
        pattern = f"{self.key_prefix}:execution:*:record"
        while True:
            cursor, keys = await self.redis.scan(cursor=cursor, match=pattern, count=100)
            now = utc_now().isoformat()
            for record_key in keys:
                retired += int(
                    await self._scripts.retire_incompatible(
                        keys=[
                            record_key,
                            self.delayed_key,
                            self.state_counts_key,
