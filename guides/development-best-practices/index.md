...
See [Environment Variables Reference](https://deepset-ai.github.io/hayhooks/reference/environment-variables/index.md) for all available settings.
...
1. Test via the API or Swagger UI
1. Repeat

Info

`--overwrite` and `--skip-saving-files` work with both `deploy-files` and `deploy-yaml`. See [CLI Commands](https://deepset-ai.github.io/hayhooks/features/cli-commands/index.md) for the full flag reference.
...
Hayhooks loads `.env` automatically on startup. See [Configuration](https://deepset-ai.github.io/hayhooks/getting-started/configuration/index.md) for all configuration methods and [Code Sharing](https://deepset-ai.github.io/hayhooks/advanced/code-sharing/index.md) for organizing shared code across pipeline wrappers.
...
1. **Verify imports** -- a missing or misspelled import in `pipeline_wrapper.py` is the most common deploy failure. The traceback will show the exact `ImportError`.
1. **Check the wrapper signature** -- `setup()` must take no arguments (besides `self`), and `run_api()` / `run_api_async()` must match the expected parameter types. See [PipelineWrapper](https://deepset-ai.github.io/hayhooks/concepts/pipeline-wrapper/index.md) for the full API.
1. **Inspect the pipeline YAML** -- if you use a `pipeline.yml`, make sure component names and connections are valid. Deploy with `LOG=DEBUG` to see the Haystack deserialization output.
1. **Test components in isolation** -- run individual Haystack components in a plain Python script before wiring them into a wrapper.
...
See [API Reference](https://deepset-ai.github.io/hayhooks/reference/api-reference/index.md) for the complete endpoint documentation.
...
If you see the entire response arrive at once instead of streaming, check that your pipeline wrapper returns a streaming generator. See [OpenAI Compatibility](https://deepset-ai.github.io/hayhooks/features/openai-compatibility/index.md) for implementation details on both APIs.

## Next Steps

- [PipelineWrapper](https://deepset-ai.github.io/hayhooks/concepts/pipeline-wrapper/index.md) -- Full wrapper API and development workflow
- [CLI Commands](https://deepset-ai.github.io/hayhooks/features/cli-commands/index.md) -- All CLI flags and commands
- [Configuration](https://deepset-ai.github.io/hayhooks/getting-started/configuration/index.md) -- Configuration methods and examples
- [Production Best Practices](https://deepset-ai.github.io/hayhooks/guides/production-best-practices/index.md) -- Hardening your deployment for production
